  ...
  if (
    ...
    try {
      ...
      if (
        ...
        (void 0 !== s.enable_quirks && s.enable_quirks)
      ) {
        var ua = navigator.userAgent;
        /Firefox.(\d+\.\d+)/i.test(ua) &&
          void 0 === s.xhr_ulMultistream &&
          (settings.xhr_ulMultistream = 1),
          /Edge.(\d+\.\d+)/i.test(ua) &&
            void 0 === s.xhr_dlMultistream &&
      ...
      /Edge.(\d+\.\d+)/i.test(ua) && (settings.forceIE11Workaround = !0),
        ...
        void 0 !== s.telemetry_level &&
          (settings.telemetry_level =
            "basic" === s.telemetry_level
              ? 1
              : "full" === s.telemetry_level
              ? 2
              : 0),
      ...
      runNextTest = function () {
        if (5 != testStatus)
          if (test_pointer >= settings.test_order.length)
            0 < settings.telemetry_level
              ? sendTelemetry(function (id) {
                  (testStatus = 4), -1 != id && (testId = id);
                })
              : (testStatus = 4);
          else
    ...
    ipCalled ||
      ...
      ((xhr = new XMLHttpRequest()).onload = function () {
        ...
        try {
          ...
          (clientIp = data.processedString), (ispInfo = data.rawIspInfo);
        } catch (e) {
  ...
  if ((tlog("dlTest"), !dlCalled)) {
              ...
              if (1 === testStatus) {
                ...
                (xhr[i] = x),
                  (xhr[i].onprogress = function (event) {
                    if (
                      (tlog(
                        "dl stream progress event " + i + " " + event.loaded
                      ),
                      1 !== testStatus)
                    )
                      try {
    ...
    interval = setInterval(
      function () {
        ...
        (graceTimeDone && (dlProgress = t / (1e3 * settings.time_dl)),
        t < 200) ||
          (graceTimeDone
            ? ((dlStatus = (
                (8 *
                  (totLoaded / (t / 1e3)) *
                  settings.overheadCompensationFactor) /
                (settings.useMebibits ? 1048576 : 1e6)
              ).toFixed(2)),
            ...
            : t > 1e3 * settings.time_dlGraceTime &&
              (0 < totLoaded &&
                ((startT = new Date().getTime()), (totLoaded = 0)),
              (graceTimeDone = !0)));
      }.bind(this),
      200
    );
  ...
  if ((tlog("ulTest"), !ulCalled)) {
        ...
        function () {
          if (3 === testStatus) {
            ...
            var ie11workaround,
              prevLoaded = 0,
              x = new XMLHttpRequest();
            if (((xhr[i] = x), settings.forceIE11Workaround))
              ie11workaround = !0;
            else
              try {
    ...
    interval = setInterval(
      function () {
        ...
        var t = new Date().getTime() - startT;
        (graceTimeDone && (ulProgress = t / (1e3 * settings.time_ul)),
        t < 200) ||
          (graceTimeDone
            ? ((ulStatus = (
                (8 *
            ...
            : t > 1e3 * settings.time_ulGraceTime &&
              (0 < totLoaded &&
                ((startT = new Date().getTime()), (totLoaded = 0)),
              (graceTimeDone = !0)));
      }.bind(this),
      ...
      tlog("ping"),
        ...
        (xhr[0].onload = function () {
          if ((tlog("pong"), 0 === i)) prevT = new Date().getTime();
            ...
            1 === i
              ? (ping = instspd)
              : ((ping = instspd < ping ? instspd : 0.8 * ping + 0.2 * instspd),
                (jitter =
                  2 === i
                    ? instjitter
                    : jitter < instjitter
                    ? 0.3 * jitter + 0.7 * instjitter
                    : 0.8 * jitter + 0.2 * instjitter)),
              (prevInstspd = instspd);
          }
          (pingStatus = ping.toFixed(2)),
            ...
            i < settings.count_ping ? doPing() : ((pingProgress = 1), done());
        }.bind(this)),
        (xhr[0].onerror = function () {
          tlog("ping failed"),
            0 === settings.xhr_ignoreErrors &&
              ((jitterStatus = pingStatus = "Fail"), clearRequests(), done()),
            1 === settings.xhr_ignoreErrors && doPing(),
            2 === settings.xhr_ignoreErrors &&
              (++i < settings.count_ping ? doPing() : done());
        }.bind(this)),
        xhr[0].open(
          "GET",
          settings.url_ping + url_sep(settings.url_ping) + "r=" + Math.random(),
          !0
        ),
        xhr[0].send();
    }.bind(this);
...
function sendTelemetry(done) {
  if (!(settings.telemetry_level < 1)) {
    ((xhr = new XMLHttpRequest()).onload = function () {
      ...
      (xhr.onerror = function () {
        console.log("TELEMETRY ERROR " + xhr), done(-1);
      }),
      xhr.open(
        "POST",
